...
exports.default = {
    ...
    findAll: function (req, res) {
        models_1.default.Question.find(req.query)
            .sort({ date: -1 })
    ...
    findById: function (req, res) {
        models_1.default.Question.findById(req.params.id)
            .then(function (dbModel) { return res.json(dbModel); })
            .catch(function (err) { return res.status(422).json(err); });
    },
    //to create a new questions -> user and admin
    create: function (req, res) {
        models_1.default.Question.create(req.body)
            .then(function (dbModel) { return res.json(dbModel); })
            .catch(function (err) { return res.status(422).json(err); });
    },
    //to update an existing question -> Admin only
    update: function (req, res) {
        models_1.default.Question.findOneAndUpdate({ _id: req.params.id }, req.body)
            .then(function (dbModel) { return res.json(dbModel); })
            .catch(function (err) { return res.status(422).json(err); });
    },
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, models_1.default.Question.findById({ _id: req.params.id })];
                    case 1:
                        dbModel = _a.sent();
                        if (dbModel) {
                            dbModel.remove();
